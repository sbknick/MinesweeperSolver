namespace MinesweeperSolver.Tests.Tricks
{
    ...
    public class OneNTrickTests
    {
        public class TestCase
        {
            public TestMap Map { get; set; }
            public (int x, int y) TriggerIndex { get; set; }
            public (int x, int y) ExpectedFlaggedIndex { get; set; }
        }

        public class TestCasePlural : TestCase
        {
            public new (int x, int y)[] ExpectedFlaggedIndex { get; set; }
        }
        ...
        private static IEnumerable<TestCasePlural> GoodPluralTestCases()
        {
            /*  +-+-+-+
             *  | |1|#|
             *  | |3|#|
             ...
             *  +-+-+-+
             */
            yield return new TestCasePlural
            {
                Map = new TestMap
                {
                    SquaresGrid = new ISquare[]
                    {
                        ...
                        new TestSquare { Index = (1, 2), IsNumber = true, Number = 9 },
                        new TestSquare { Index = (2, 2), IsNumber = true, Number = 3 },
                        new TestSquare { Index = (3, 2), IsBlank = true },

                        new TestSquare { Index = (1, 3), IsNumber = true, Number = 9 },
                        new TestSquare { Index = (2, 3), IsBlank = true },
                        new TestSquare { Index = (3, 3), IsBlank = true },
                    }.ToDictionary(x => x.Index),
                },
                TriggerIndex = (2, 1),
                ExpectedFlaggedIndex = new (int x, int y)[]
                {
                    (2, 3),
                    (3, 3),
                },
            ...
            yield return new TestCasePlural
            {
                Map = new TestMap
                {
                    SquaresGrid = new ISquare[]
                    {
                        ...
                        new TestSquare { Index = (1, 2), IsNumber = true, Number = 9 },
                        new TestSquare { Index = (2, 2), IsNumber = true, Number = 4 },
                        new TestSquare { Index = (3, 2), IsBlank = true },
        ...
        private static IEnumerable<TestCase> CombinedCases => GoodTestCases().Union(GoodPluralTestCases());
        ...
        public void Predicate_ShouldReturnTrueFalseCorrectly(TestCase testCase)
        {
            ...
            var trick = new OneNTrick(mapHandler);
        ...
        public void DoTheThing_ShouldFlagTheCorrectSquares(TestCase testCase)
        {
            ...
            Assert.AreEqual(expectedSquare, processedSquares.First());
        }
        ...
        [Test,  TestCaseSource(nameof(GoodPluralTestCases))]
        public void DoTheThing_ShouldFlagTheCorrectSquares(TestCasePlural testCase)
        {
            // Arrange
            var mapHandler = new MapHandler(testCase.Map);
            var trick = new OneNTrick(mapHandler);
            ...
            // Assert
            Assert.AreEqual(testCase.ExpectedFlaggedIndex.Length, processedSquares.Count);
            Assert.IsEmpty(newNumbersToProcess);
            var expectedSquares = testCase.ExpectedFlaggedIndex.Select(idx => mapHandler.GetSquare(idx));
            Assert.IsTrue(expectedSquares.All(sq => sq.IsFlagged));
            Assert.AreEqual(expectedSquares, processedSquares);
        }
    }
}
